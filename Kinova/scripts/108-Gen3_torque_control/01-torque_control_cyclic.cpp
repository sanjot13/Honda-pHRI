...
//                std::cout << "tool_external_wrench_force_z " << base_feedback.base().tool_external_wrench_force_z() << " N" << std::endl;
                 ...
                 * (base_feedback.actuators(actuator_count - 1).torque() - 10*init_last_torque)
                 *
                 * can try larger scalar instead of 0 + init_first_torque
                 */
                base_command.mutable_actuators(0)->set_torque_joint(6 + 1*init_first_torque + (torque_amplification * (base_feedback.actuators(actuator_count - 1).torque() - init_last_torque)));
